...
_DEFAULT_COMPILER_SETTINGS = [
    ...
    ('blockMarkerEnd', ('\n<!-- END BLOCK: ', ' -->\n'), ''),
    ('defDocStrMsg',
     'Autogenerated by Cheetah: '
     'The Python-Powered Template Engine  # noqa: E501', ''),
...
class MethodCompiler(GenUtils):
    ...
    def addFilteredChunk(self, chunk, filterArgs=None,
        ...
        if self.setting('alwaysFilterNone'):
            if rawExpr and rawExpr.find('\n') == -1 and \
                    rawExpr.find('\r') == -1:
                self.addChunk(
                    "_v = %s  # noqa: E501; %r" % (chunk, rawExpr))
                if lineCol:
    ...
    def addPlaceholder(self, expr, filterArgs, rawPlaceholder,
        ...
        if self.setting('outputRowColComments'):
            self.appendToPrevChunk(
                '  # noqa: E501; from line %s, col %s.' % lineCol)
        if cacheInfo:
            self.endCacheRegion()
    ...
    def addIndentingDirective(self, expr, lineCol=None):
        ...
        if lineCol:
            self.appendToPrevChunk(
                '  # noqa: E501; Generated from line %s, col %s' % lineCol)
        self.indent()
    ...
    def startCacheRegion(self, cacheInfo, lineCol, rawPlaceholder=None):
        ...
        self.addChunk('_cacheRegion_%(ID)s = self.getCacheRegion(' % locals())
        self.indent()
        self.addChunk('regionID='
                      + repr(ID)
    ...
    def setFilter(self, theFilter, isKlass):
        ...
        if isKlass:
            ...
            self.indent()
            self.addChunk(theFilter.strip() + '(self).filter')
            self.dedent()
        else:
            if theFilter.lower() == 'none':
                ...
                self.indent()
                self.addChunk('self._CHEETAH__filters[filterName] = \\')
                self.addChunk('getattr(self._CHEETAH__filtersLib,')
                self.addChunk('        filterName)(self).filter')
                self.dedent()
                self.dedent()
...
class AutoMethodCompiler(MethodCompiler):
    ...
    def _addAutoSetupCode(self):
        ...
        if self._streamingEnabled and \
            ...
            self.indent()
            self.indent()
            self.addChunk('and not callable(self.transaction)):')
            self.dedent()
...
def __str__(self):
    ...
    def closeBlock(self):
        ...
        # self.appendToPrevChunk(
        # '  # noqa: E501; Generated from ' + repr(rawDirective) )
        # if self.setting('outputRowColComments'):
